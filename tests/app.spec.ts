...
test('service worker file is served at /sw.js and references our shell assets', async ({
  ...
  expect(body).toContain('food-log-v1-9');
...
// ─── v1.9 voice speed lane (long-press ➕ Meal) ─────────────────────────────
//
// Playwright can't drive the real Web Speech API, so we install a mock
// SpeechRecognition on `window` before page scripts run. The mock exposes a
// `__pushTranscript()` helper we can call via page.evaluate() to simulate
// live transcript chunks arriving from the browser.
...
async function installSpeechMock(page: Page): Promise<void> {
  await page.addInitScript(() => {
    ...
    class MockSpeechRecognition {
      ...
      maxAlternatives = 1;
      onresult: ((ev: { resultIndex: number; results: ArrayLike<MockResult> }) => void) | null =
        null;
      ...
      start(): void {
        this.started = true;
        (window as unknown as { __lastSpeechMock: MockSpeechRecognition }).__lastSpeechMock = this;
      }
      stop(): void {
      ...
      pushTranscript(text: string): void {
        const result: MockResult = { isFinal: true, 0: { transcript: text } };
    ...
    (window as unknown as { __pushTranscript: (text: string) => void }).__pushTranscript = (
      text: string
    ) => {
      const m = (window as unknown as { __lastSpeechMock?: MockSpeechRecognition })
        .__lastSpeechMock;
      if (m) m.pushTranscript(text);
    };
  });
}

// Helper: simulate a long-press by holding the pointer down for >350ms.
async function longPressMealButton(page: Page): Promise<void> {
  const btn = page.locator('#add-meal-btn');
  await btn.waitFor({ state: 'visible' });
  const box = await btn.boundingBox();
  if (!box) throw new Error('Meal button not visible');
  const cx = box.x + box.width / 2;
  const cy = box.y + box.height / 2;
  await page.mouse.move(cx, cy);
  await page.mouse.down();
  // 400ms ≥ 350ms long-press threshold.
  await page.waitForTimeout(400);
}

test('long-press ➕ Meal opens the talking panel with a pulsing dot', async ({ page }) => {
  await installSpeechMock(page);
  await page.reload();
  await longPressMealButton(page);
  await expect(page.locator('.talk-panel')).toBeVisible();
  await expect(page.locator('.talk-panel .talk-pulse')).toBeVisible();
  await expect(page.locator('.talk-panel-hint')).toContainText(/release/i);
  // Cleanup: release so the mouse isn't stuck down across tests.
  await page.mouse.up();
});

test('long-press → live transcript shows in the talking panel', async ({ page }) => {
  await installSpeechMock(page);
  await page.reload();
  await longPressMealButton(page);
  await page.evaluate(() => {
    (window as unknown as { __pushTranscript: (t: string) => void }).__pushTranscript(
      'two crackers because no appetite'
    );
  });
  await expect(page.locator('.talk-panel-transcript')).toContainText(
    'two crackers because no appetite'
  );
  await page.mouse.up();
});

test('releasing after a transcript saves a description-only meal with eaten_at=now', async ({
  page,
}) => {
  await installSpeechMock(page);
  await page.reload();
  ...
  await page.mouse.up();
  // Panel hides, meal appears in Today list.
  await expect(page.locator('.talk-panel')).toHaveCount(0, { timeout: 2000 });
  const firstCard = page.locator('.meal-list .meal-card').first();
  await expect(firstCard).toBeVisible({ timeout: 5000 });
  await expect(firstCard.locator('.meal-card-desc')).toContainText('cottage cheese and cucumber');
  // No photo strip (voice-only flow doesn't capture photos).
  await expect(firstCard.locator('.meal-card-photos')).toHaveCount(0);
  // Time is "now" — we don't pin the exact minute but it should be HH:MM today.
  await expect(firstCard.locator('.meal-card-time')).toHaveText(/^\d{2}:\d{2}$/);
});

test('releasing with no transcript writes no meal (silent abort)', async ({ page }) => {
  await installSpeechMock(page);
  await page.reload();
  await longPressMealButton(page);
  // No transcript pushed. Release.
  await page.mouse.up();
  await expect(page.locator('.talk-panel')).toHaveCount(0, { timeout: 2000 });
  // Still empty Today (no meal saved).
  await expect(page.locator('.card').first().locator('.today-empty')).toContainText(
    'No meals yet today'
  );
});

test('short tap on ➕ Meal still opens the existing sheet (no voice mode)', async ({ page }) => {
  await installSpeechMock(page);
  await page.reload();
  // Plain click — well under the 350ms long-press threshold.
  await page.locator('#add-meal-btn').click();
  await expect(page.locator('.sheet-panel')).toBeVisible();
  // And the talk panel is NOT shown for a tap.
  await expect(page.locator('.talk-panel')).toHaveCount(0);
});

test('Web Speech API unavailable → long-press falls back to opening the sheet', async ({
  page,
}) => {
  // Explicitly do NOT install the speech mock. The page sees no
  // SpeechRecognition ctor and should fall back to plain-tap behavior.
  await page.addInitScript(() => {
    delete (window as unknown as { SpeechRecognition?: unknown }).SpeechRecognition;
    delete (window as unknown as { webkitSpeechRecognition?: unknown }).webkitSpeechRecognition;
  });
  await page.reload();
  // A long-press should still open the sheet (the fallback path is bound as a
  // plain click handler in this branch).
  await page.locator('#add-meal-btn').click();
  await expect(page.locator('.sheet-panel')).toBeVisible();
});
